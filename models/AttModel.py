...
class AttModel(CaptionModel):
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None, input_labels=None):
        ...
        for i in range(seq.size(1) - 1):
            ...
            output, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state, i, p_fc_feats, input_labels)
            outputs[:, i] = output

        return outputs

    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, state, i=None, p_fc_feats=None, input_labels=None):
    ...
    def _sample(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        batch_size = fc_feats.size(0)
        input_labels = fc_feats.new(batch_size).zero_().long() + 1
        state = self.init_hidden(batch_size)
        ...
        for t in range(self.seq_length + 1):
            ...
            logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state, t, p_fc_feats, input_labels)
    ...
    def beam_sample(self, fc_feats, att_feats, att_masks, opt={}):
        ...
        for i in range(self.seq_length):
            ...
            output, decState = self.get_logprobs_state(inp, tmp_fc_feats, tmp_att_feats, tmp_p_att_feats, tmp_att_masks,
                                                       decState)
            ...
            t_tmp_att_feats = tmp_att_feats.view(beam_size, batch_size, tmp_att_feats.size(1), tmp_att_feats.size(2))
            t_tmp_p_att_feats = tmp_att_feats.view(beam_size, batch_size, tmp_p_att_feats.size(1), tmp_p_att_feats.size(2))
            tmp_att_masks = unbottle(tmp_att_masks)

            for j, b in enumerate(beam):
                tmp = [t_tmp_fc_feats[:, j, :], t_tmp_att_feats[:, j, :, :], t_tmp_p_att_feats[:, j, :, :], tmp_att_masks[:, j, :]]
                b.advance(output[:, j])
