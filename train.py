...
def train(opt):
    ...
    cider_crit = utils.ScoreLanguageModelCriterion()
    ...
    while True:
        ...
        if not sc_flag:
            if opt.cider:
                cider_scores = np.exp(data['cider_scores'])
                loss = cider_crit(dp_model(fc_feats, att_feats, labels, att_masks), labels[:, 1:], masks[:, 1:], cider_scores)
                print(loss.item()/np.sum(cider_scores) * labels.size(0))
            else:
        ...
        if (iteration % opt.save_checkpoint_every == 0):
            ...
            if True: # if true
                ...
                torch.save(model.state_dict(), checkpoint_path)
                print("model saved to {}".format(checkpoint_path))
                with open(os.path.join(opt.checkpoint_path, 'infos_' + opt.id + '_' + str(iteration) + '.pkl'), 'wb') as f:
                    cPickle.dump(infos, f)
