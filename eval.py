...
# Set sample options
if opt.caption_model == 'att2in2p':
    loss, split_predictions, lang_stats = eval_utils_pair.eval_split(model, crit, loader,
                                                                vars(opt))
else:
...
if opt.dump_json == 1:
    # dump the json
    if opt.beam_size == 1:
        json.dump(split_predictions, open('vis/' + opt.id + '.json', 'w'))
    else:
        json.dump(split_predictions, open('vis/' + opt.id + '_beam' + str(opt.beam_size) + '.json', 'w'))
